...
# rest of dynamics, calculation 
        ...
        if done: 
            ...
            print("====> Episode: {},Spd:{}, friction: {},size:{},loc:{},Reward: {}, Stop_Dist: {}".format(self.episode,self.kickspd,self.friction_of_patch,self.size_of_patch,self.location_of_patch,reward, groundtruth_distance))
            if self.collect["option"] != 0:
               self.collect_data(self.episode, self.kickspd, self.friction_of_patch,self.default_friction,self.location_of_patch,self.size_of_patch, reward,groundtruth_distance)
            self.episode+=1
        else:
            reward = 0
