...
class ddpgAgent():
    def __init__(self, Testing=False):
        ...
        try:
            self.actor.model.load_state_dict(torch.load("./models/controller/intermittent/actor3000.pt"))
            self.actor.target_model.load_state_dict(torch.load("./models/controller/intermittent/actor3000.pt"))
    ...
    def getAction(self, state, epsilon):
        ...
        if self.testing is False:
            noise[0][0] = (1.0-float(self.testing)) * max(epsilon, 0) * self.OU.function(action_original[0][0], 0.2, 1.0, 0.10)
